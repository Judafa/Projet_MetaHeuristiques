...
class Algo_Tabou:
    ...
    def __init__(self, Rcapt, Rcom, k, filepath):
        ...
        for cible1 in cibles:
            for cible2 in cibles:
                if (self.distance(cible1, cible2) <= Rcapt and cible1 != cible2 and cible2 != (
                        "0.00", "0.00")):  # On n'inclut pas la cible elle-même dans sa liste des voisins
                    self.ajouter_elem_a_dict_type1(self.adjacence_capt, cible1, cible2)
                    self.ajouter_elem_a_dict_type1(self.adjacence_com, cible1, cible2)
                elif (self.distance(cible1, cible2) <= Rcom and cible1 != cible2 and cible2 != (
                        "0.00", "0.00")):  # On n'inclut pas la cible elle-même dans sa liste des voisins
                    self.ajouter_elem_a_dict_type1(self.adjacence_com, cible1, cible2)
    ...
    def run(self, taille_tabou, nb_iterations):
        ...
        C, lc, ld = self.parcours()
        print("Réalisabilité de la solution initiale : " + str(self.vérifier_réalisabilité(C)))
        print("\n")
        preds, succs = self.construire_chemin(C)
        nouv_C = self.post_traitement(preds, succs, C)
        print("Réalisabilité de la solution après traitement : " + str(self.vérifier_réalisabilité(nouv_C)))
        print(self.vérifier_réalisabilité(nouv_C))
        print("\n")
        time1 = time.time()
        meilleur_chemin, minimum, i, lc, minimums = self.parcours_voisinages(nouv_C, taille_tabou, nb_iterations)
        time2 = time.time()
        print(
    ...
    def parcours(self):
        ...
        for elem in self.adjacence_com[p]:
            qualites_voisins_p[elem] = self.qualite(elem, self.adjacence_capt, D)
        ...
        for cible in self.adjacence_com[("0.00", "0.00")]:  # Pour chaque voisin du puits dans Gcom on incrémente le nombre de capteurs avec lesquels il peut communiquer dans cibles_vois_c_com car le but c'est de communiquer avec le puits
            self.ajouter_elem_a_dict_type2(cibles_vois_c_com, cible)
        ...
        while len(D) != n:  # Tant qu'il y a des cibles non k-couvertes
            ...
            if (cibles_visitées[
                cible_courante] == 1):  # Si la cible courante a été visitée (c.à.d elle a été k-couverte donc on va passer à une autre)
                v = self.adjacence_capt[cible_courante][
                    0]  # premier candidat à remplacer la cible courante (son premier voisin dans la liste d'adjacence de captation)
                        ...
                        for elem in voisins_capt:
                            ...
                            if elem not in C:  # On cherche la cible de meilleure qualité pour mettre le capteur alors on doit vérifier qu'il n'y a pas un capteur déjà installé sur cette cible
                                ...
                                if cibles_vois_c_com[elem] > 0:  # pour assurer la communication (ceci signifie que pour l'élément considéré il a au moins un voisin de communication dans C ou il peut communiquer directement avec le puits)
                                    ...
                                    voisins_qualités[elem] = self.qualite(elem, self.adjacence_capt, D)
    ...
    def post_traitement(self, preds, succs, C):
        nouveau_C = C.copy()

        for capteur in C:  # Pour chaque capteur dans C
            inutile = True
            if self.degre_couv(capteur, nouveau_C) > self.k:  # si le capteur est couvert par un nombre de capteurs > k
                for cible in self.adjacence_capt[capteur]:  # pour chaque cible captée par ce capteur
                    if self.degre_couv(cible, nouveau_C) == self.k:  # si on a une cible qui est captée exactement par k capteurs alors ce capteur est utile
                        inutile = False
                        break
    ...
    def construire_chemin(self, C):
        ...
        while (nombre_restant > 0):  # tant qu'il y a des capteurs non ajoutés dans le chemin
            ...
            if not ajouté[C[i]]:  # si le capteur i n'est pas ajouté
                if C[i] in self.adjacence_com[("0.00", "0.00")]:  # s'il peut communiquer directement avec le puits
                    ...
                    for voisin in self.adjacence_com[C[i]]:  # pour chaque voisin de communication de ce ce capteur
                        ...
                        if voisin in C:  # si le voisin est dans C
                            ...
                            if ajouté[voisin]:  # et si le voisin a été ajouté dans le chemin
                                ...
                                succ[voisin].append(
                                    C[i])  # et on ajoute le capteur à la liste des successeurs du voisin
    ...
    captant cette cible.'''
    def supprimer_ajouter(self, capteur, C, pred, succ):
        ...
        for voisin_capt in self.adjacence_capt[capteur]:  # on choisit le nouveau capteur parmi les voisins du capteur à supprimer
            ...
            if voisin_capt not in C:  # si le voisin n'est pas dans C
                ...
                if (pred[capteur] in self.adjacence_com[voisin_capt] or voisin_capt in self.adjacence_com[("0.00",
                                                                                     "0.00")]):  # si le voisin peut communiquer avec le prédécesseur du capteur à supprimer ou directement avec le puits (on peut faire mieux)
            ...
            if valide == True:  # si le voisin conserve la communication
                for elem in à_couvrir:  # pour tout élément de à couvrir
                    ...
                    if (elem not in self.adjacence_capt[
                        voisin_capt] and elem != voisin_capt):  # si pour un élément différent du voisin cet élément n'est pas couvert par le voisin
                        valide = False  # alors le voisin n'est pas valide
                        break
    ...
    def voisinage(self, C, pred, succ):
        ...
        capteurs_supprimés = []
        for capteur in C:  # pour chaque capteur dans C
            existe, nouv_C = self.supprimer_ajouter(capteur, C, pred, succ)  # on effectue la transformation supprimer_ajouter
    ...
    def parcours_voisinages(self, C, taille_tabou, nb_itérations):
        liste_tabou = []
        i = 0
    ...
    def vérifier_réalisabilité(self, C):
        ...
        for elem in C:
            for elem2 in self.adjacence_com[elem]:
                if (elem2 in C):
                    a.append(elem)
                    break
            if elem in self.adjacence_com[("0.00", "0.00")]:
                a.append(elem)
        if set(a) != set(C):
            print("non comm")
            return False
    ...
    def vérifier_cnc(self, C, cnc, liste_cibles):
        a = True
    ...
    def qualite(self, cible, adj_capt, D):
        qual = 0
